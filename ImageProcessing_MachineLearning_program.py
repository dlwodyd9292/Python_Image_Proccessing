...
def colorExt_cv() : # 상하 대칭
    ...
    #####실제 영상처리########################################
    hsv = cv2.cvtColor(inCvImage, cv2.COLOR_BGR2HSV) # RGB를 HSV로 변환
    h, s, v = cv2.split(hsv)
    h_orange = cv2.inRange(h, 8, 20) # 주황 hue 값 : 8 ~ 20

    outCvImage = cv2.bitwise_and(hsv, hsv, mask = h_orange) # mask 색상 외에 전부 검게
    outCvImage = cv2.cvtColor(outCvImage, cv2.COLOR_HSV2RGB) # 모니터 출력은 RGB만 가능
    #########################################################
    ## OpenCV --> outImage 영상처리
    ## (중요!) 입력 이미지의 폭, 높이 알아내기
    cv2output()
...
openCVMenu2.add_command(label="머신러닝(눈인식)", command=eyeDetect_cv)
